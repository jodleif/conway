...
bool conway::game_of_life::cell_change_rule(rule_neighbor_state ruling, bool curr_state)
{
    ...
    case rule_neighbor_state::invalid:
        std::cerr << "[game_of_life::cell_change_rule] ERROR: Got invalid neighbor state\n";
        return false;
    }
...
bool conway::game_of_life::alive(int x, int y)
{
    ...
    return (_game_board[coord_to_pos(x, y)] != 0);
}
...
int conway::game_of_life::get_width()
{
    return _width;
}

int conway::game_of_life::get_height()
{
    return _height;
}

void conway::game_of_life::reset_board()
{
    std::fill(_game_board.begin(),_game_board.end(),0);
}
